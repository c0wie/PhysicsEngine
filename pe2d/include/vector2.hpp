...
/// @brief Class represeting two component vector.
/// @details Vector2<T> serves multiple roles in pe2d - not just as a mathematical vector,
///          but also for any case requiring two components. Common uses include:
...
public:
  ...
  /// @return True if x and y components are the same for both vectors, false otherwise
  ...
  /// @param other - Vector2 to compare with 
  /// @return True if at least one components of vectors is different, false otherwise
  constexpr bool operator!=(Vector2 other) const {
    return x != other.x || y != other.y;
  }
  ...
  /// @warning Throws exception on division by zero - consider checking components before use
  ...
  /// @warning Throws exception on division by zero - consider checking scalar before use
